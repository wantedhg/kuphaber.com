				...
				for($i = 0; $i < $adet; $i++)
				{
					...
					if($text == '')
					{
						...
						if($html <> '')
						{
							...
							//salt metin olarak alalım
							$text = $html->plaintext;

							//devamını oku yazısını silelim
							$text = str_replace('Devamı için tıklayınız','',$text);
							//bellek boşaltıyoruz
							$html->clear();
							unset($html);
						}
					...
					if($text == '')
					{
						...
						$exploded = explode('?u=', $text);
						if($exploded[1] <> '')
						{
							$list[$i]['cache_image'] = trim($exploded[1]);
						}
					}

					//sunucu saati farkı sebebiyle rss tarihleri hatalı
					//onları bizim tarih dilimine çevirelim
					$list[$i]['cache_time'] = convert_to_turkish_time( $list[$i]['cache_time'], $hour = '-1 hour');
				}
			}
			...
			if($adet > 0)
			{
				...
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if(!in_array($list[$i]['cache_link'], $list_url))
					{
						...
						$array_url  = array(
							'http://www.teknokulis.com/multimedya/',
						);

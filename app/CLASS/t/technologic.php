	...
	class technologic
	{
		public function rss_fetch($url, $type)
		{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					//metinde düzenleme yap
					$list[$i]['cache_desc'] 		= str_replace('[&#038;hellip', '', $list[$i]['cache_desc']);
				}
			}
			...
			if($adet > 0)
			{
				...
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if(!in_array($list[$i]['cache_link'], $list_url))
					{
						...
						$array_url  = array(
							'http://www.technologic.com.tr/yazarlar/',
						);
		...
		private function data_fetch($data, $type = 'image')
		{
			...
			if($type == 'image')
			{
				...
				//resim yolundan wp cacher linkini kaldıralım
				$text = str_replace(array('i0.wp.com/','i1.wp.com/','i2.wp.com/'),'',$text);

				//https yolunu http yapalım

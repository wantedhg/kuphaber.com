		...
		public function rss_fetch($url, $type)
		{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					//başlıkta çifte boşluk bulunmasın + Hepsi de büyük olmasın
					$list[$i]['cache_title']		= tr_ucwords(str_replace('  ',' ',$list[$i]['cache_title']));

					//resim linkindeki saçmalık düzeltilsin
					$list[$i]['cache_image']		= urlencode(str_replace(array('.jpgk.jpg','http://www.airkule.com/haber_resim/'),'',$list[$i]['cache_image']));
					$list[$i]['cache_image'] 		= 'http://www.airkule.com/haber_resim/'.$list[$i]['cache_image'].'.jpg';
				}
			}

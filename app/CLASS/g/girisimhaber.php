			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					if($text == '')
					{
						...
						//değişkene atıyoruz
						$list[$i]['cache_desc'] = trim(html_entity_decode($text));
					}
					...
					if($text == '')
					{
						...
						if($html <> '')
						{
							...
							//url eksik, düzeltelim
							if($text <> '')
							{
								$text = str_replace('/uploads','http://www.girisimhaber.com/uploads',$text);
							}
						}
			...
			if($adet > 0)
			{
				...
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if(!in_array($list[$i]['cache_link'], $list_url))
					{
						...
						$array_url  = array(
							'http://www.girisimhaber.com/yazarlar/',
						);
						...
						if($hata <> 1)
						{
							...
							//ekliyoruz
							//print_pre($list[$i]);
							//içerik resminde query bulunmasın
							//resim uzantısı belli bir formata zorlanmasın

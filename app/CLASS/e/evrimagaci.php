		...
		public function rss_fetch($url, $type)
		{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					//delta taglarını temizliyoruz
					$list[$i]['cache_link'] 		= str_replace('delta.evrimagaci.org','evrimagaci.org',$list[$i]['cache_link']);
					$list[$i]['cache_image'] 		= str_replace('delta.evrimagaci.org','evrimagaci.org',$list[$i]['cache_image']);
			...
			if($adet > 0)
			{
				...
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if(!in_array($list[$i]['cache_link'], $list_url))
					{
						...
						$array_url  = array(
							'http://www.evrimagaci.com/yazarlar/',
						);

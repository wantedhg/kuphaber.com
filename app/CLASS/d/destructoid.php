<?php
	...
	class destructoid
	{
		public function rss_fetch($url, $type)
		{
			...
			//http linkleri https e dönüştürüyoruz
			$data_curl = str_replace('http://','https://',$data_curl);
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					$list[$i]['cache_link']			= trim(strip_tags($data->entry[$i]->link->attributes()->href));
					$list[$i]['cache_desc']			= trim(strip_tags($data->entry[$i]->content));
			...
			if($adet > 0)
			{
				...
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if(!in_array($list[$i]['cache_link'], $list_url))
					{
						...
						$array_url  = array(
							'canli-izle',
							'http://www.destructoid.com/yazarlar/',
						);

<?php
	...
	/**
	* Hibrit Model: Sadece RSS Parse ederek imaj veya data elde edemezsek
	* Detay sayfadaki head bilgilerinden bu bilgiyi çıkartabiliriz
	* Bu model biraz daha ilk yazdığımız RSS mantığına yaklaşıyor;
	* Lakin yine de Full Metin almaya çalışmıyor
	*/
			...
			if($adet > 0)
			{
				...
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if(!in_array($list[$i]['cache_link'], $list_url))
					{
						...
						$array_url  = array(
							'canli-izle',
							'http://www.milscint.com/tr/yazarlar/',
						);

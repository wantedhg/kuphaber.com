<?php
	...
	class kotaku
	{
		public function rss_fetch($url, $type)
		{
			...
			//2. data reklam sabiti kaldırıyorum
			unset($data->item[1]);
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					$list[$i]['cache_desc'] 		= str_replace('Read more...', '', $list[$i]['cache_desc']);
			...
			if($adet > 0)
			{
				...
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if(!in_array($list[$i]['cache_link'], $list_url))
					{
						...
						//sadece belli url mantıklarına izin VERİYORUZ
						$array_url  = array(
							'kotaku.com',
						);
						if(strpos_array($list[$i]['cache_link'], $array_url) == false)
						{
							$hata = 1;
						}

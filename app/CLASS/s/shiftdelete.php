<?php
	...
	class shiftdelete
	{
		private function turkish_date_to_global($t)
		{
			/**
			Çeşitli kaynaklarda karşılaştığımız bir acayipliki düzeltir
			rss'e türkçe olarak basılmış tarihleri global uyumlu hale getirir
			*/

			$t = str_replace(array('Pts,','Sal,','Çar,','Per,','Cum,','Cts,','Paz,','Prş,','Çrş,'),'',$t);
		...
		public function rss_fetch($url, $type)
		{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					$list[$i]['cache_time']			= strip_tags(self::turkish_date_to_global($data->item[$i]->pubDate));

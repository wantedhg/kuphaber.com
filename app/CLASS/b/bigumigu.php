		...
		public function rss_fetch($url, $type)
		{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					$list[$i]['cache_link']			= trim(strip_tags($data->item[$i]->guid));
					...
					//saçmalık kaldır
					$list[$i]['cache_desc'] 		= str_replace('Yazının tümünü okumak için tıklayın.', '', $list[$i]['cache_desc']);
					...
					if($text == '')
					{
						...
						//sadece belli tarzdaki linke izin verelim
						if(strpos($text, 'bigumigu.com/uploads') !== false)
						{
							//değişkene atıyoruz
			...
			if($adet > 0)
			{
				...
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if(!in_array($list[$i]['cache_link'], $list_url))
					{
						...
						$array_url  = array(
							'http://www.bigumigu.com/yazarlar/',
						);

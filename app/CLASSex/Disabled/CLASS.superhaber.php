		...
		public function rss_fetch($url, $type)
		{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					//superhabertv kategori servisi yok, default atıyoruz
					$list[$i]['cache_cat']			= 0;

					//desc alanında gereksiz imajlar geliyor, kaldıralım
					if($list[$i]['cache_desc'] == '')
					{
						$html = str_get_html($data->item[$i]->description);

						foreach($html->find('div.body') as $e) $text = $e->plaintext;
					...
					//resim gelmiyor, desc alanından parse ediyoruz

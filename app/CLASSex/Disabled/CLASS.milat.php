<?php
	...
	class milat
	{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if($_cache->content_cached($list[$i]['link']) == false)
					{
						...
						if(strpos_array($list[$i]['link'], array('http://www.milatgazetesi.com/')) == true)
						{
							$parser	= 'milat_www';
						}
						...
						$array_url  = array(
							'http://www.milatgazetesi.com/fotogaleri/',
							'http://www.milatgazetesi.com/video/',
						);
						...
						if($hata <> 1)
						{
							...
							$record = array(
								...
								'cache_object'	=> 'milat',		//hangi objeyi kullanacağı
		...
		public function milat_www($data, $type = 'text')
		{
			if($data == '') return '';
			...
			if($type == 'text')
			{
				...
				foreach($html->find('ul.haberList') as $e) 	$yazilar = $e->innertext;

				//yazarın köşe yazılarına bir atif varsa;
				//bu bir köşe yazısıdır,
				//parse etmeye uğraşmayalım
				if($yazilar <> '')
				{
					$text = '';
				}
				else
				{
					...
					foreach($html->find('div#detaytext b[1]') as $e) 		$e->outertext = '';
					foreach($html->find('img.haberresmi') as $e) 			$e->outertext = '';
					foreach($html->find('div.addthis_toolbox') as $e) 		$e->outertext = '';
					foreach($html->find('iframe#fasasdasd') as $e) 			$e->outertext = '';
					foreach($html->find('div.oncekisonraki') as $e) 		$e->outertext = '';
					foreach($html->find('ul.haberList') as $e) 				$e->outertext = '';

					//en son metni alalım
					foreach($html->find('div#detaytext') as $e) 			$text = $e->innertext;

					$text = preg_replace('/<div style="height:80px;">([\w\W]*?)<\/div>/', '', $text);
					$text = preg_replace('/<!-- AddThis Button BEGIN -->([\w\W]*?)<!-- AddThis Button END -->/', '', $text);
			...
			if($type == 'title')
			{
				...
				//h1 bulalım
				foreach($html->find('div.haberbasligi') as $e) $text = $e->plaintext;
			...
			if($type == 'desc')
			{
				...
				//h2 bulalım
				foreach($html->find('div#detaytext b[1]') as $e) $text = $e->plaintext;
			...
			if($type == 'image')
			{
				...
				//h2 bulalım
				foreach($html->find('img.haberresmi') as $e) $text = $e->src;

<?php
	...
	class takvim
	{
		...
		public function rss_fetch($url, $channel = 1)
		{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					$tlink = str_replace('http://www.takvim.com.tr/','', $list[$i]['link']);
					$tlink = explode('/', $tlink);
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					if($_cache->content_cached($list[$i]['link']) == false)
					{
						...
						if(strpos_array($list[$i]['link'], array('http://www.takvim.com.tr/')) == true)
						{
							$parser	= 'takvim_www';
						}
						...
						$array_url  = array(
							...
							'http://www.takvim.com.tr/multimedya/',
							'http://www.takvim.com.tr/astroloji?',
						...
						if($hata <> 1)
						{
							...
							$record = array(
								...
								'cache_object'	=> 'takvim',		//hangi objeyi kullanacağı
		...
		public function takvim_www($data, $type = 'text')
		{
			if($data == '') return '';
			...
			if($type == 'text')
			{
				...
				//metin yoksa alternatif alana bakalım
				if($text == '')
				{
					foreach($html->find('p#page-description') as $e)	$text = $e->innertext;
				}
			...
			if($type == 'title')
			{
				...
				if($text == '')
				{
					...
					//h1 bulalım
					foreach($html->find('div.header h1') as $e) $text = $e->plaintext;
			...
			if($type == 'desc')
			{
				...
				if($text == '')
				{
					...
					//h2 bulalım
					foreach($html->find('div.header h2') as $e) $text = $e->plaintext;
			...
			if($type == 'image')
			{
				...
				if($text == '')
				{
					...
					foreach($html->find('img#haberImg') as $e) $text = $e->src;

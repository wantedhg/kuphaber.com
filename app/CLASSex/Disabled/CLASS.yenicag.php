<?php
	...
	class yenicag
	{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					if($_cache->content_cached($list[$i]['link']) == false)
					{
						...
						if(strpos_array($list[$i]['link'], array('http://www.yenicaggazetesi.com.tr')) == true)
						{
							$parser	= 'yenicag_www';
						}

						//belli url mantıklarına izin vermiyoruz
						//galeri içerik veya köşe yazısı dönüyorlar
						if
						(
							(substr($list[$i]['link'], -6) == 'yy.htm')
							OR
							(substr($list[$i]['link'], -5) == 'g.htm')
						)
						{
							$hata = 1;
						}
						...
						//belli url mantıklarına izin vermiyoruz
						//resmi ilanlar veya istemediğimiz içerik dönüyorlar
						$array_url  = array(
							'-resmi-ilanlar-',
							'http://www.yenicaggazetesi.com.tr/konuk-kalem',
							'http://www.yenicaggazetesi.com.tr/medya-polemik',
						);
						...
						if($hata <> 1)
						{
							...
							$record = array(
								...
								'cache_object'	=> 'yenicag',		//hangi objeyi kullanacağı
		...
		public function yenicag_www($data, $type = 'text')
		{
			global $list_cat;
			...
			if($type == 'text')
			{
				...
				//foreach($html->find('div.main-col div.main-col') as $e)	$e->outertext = '';

				//en son metni alalım
				foreach($html->find('div#news_content') as $e)				$text = $e->innertext;
			...
			if($type == 'image')
			{
				...
				if($text == '')
				{
					...
					//h1 bulalım
					foreach($html->find('img.image') as $e) $text = $e->src;
			...
			if($type == 'cat')
			{
				...
				//h1 bulalım
				foreach($html->find('div.nav span') as $e) $text = $e->plaintext;

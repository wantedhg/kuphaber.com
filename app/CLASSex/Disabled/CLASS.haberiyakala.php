<?php
	...
	class haberiyakala
	{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					if($_cache->content_cached($list[$i]['link']) == false)
					{
						...
						if(strpos_array($list[$i]['link'], array('http://www.haberiyakala.com/')) == true)
						{
							$parser	= 'haberiyakala_www';
						}
						...
						if($hata <> 1)
						{
							...
							$record = array(
								...
								'cache_object'	=> 'haberiyakala',		//hangi objeyi kullanacağı
		...
		public function haberiyakala_www($data, $type = 'text')
		{
			if($data == '') return '';
			...
			if($type == 'text')
			{
				...
				foreach($html->find('div.all-gallery-list-caption') as $e)	$e->outertext = '';

				//en son metni alalım
				foreach($html->find('article.article-text') as $e)			$text = $e->innertext;
				//metin yoksa alternatif metni alalım
				if($text == '')
				{
					foreach($html->find('div#all-gallery') as $e)			$text = $e->innertext;
				}
			...
			if($type == 'title')
			{
				...
				//h3 bulalım
				foreach($html->find('h3') as $e) $text = $e->innertext;
			...
			if($type == 'desc')
			{
				...
				//h4 bulalım
				foreach($html->find('h4.article-spot') as $e) $text = $e->innertext;
			...
			if($type == 'image')
			{
				...
				if($text == '')
				{
					...
					foreach($html->find('figure.article-image img') as $e) $text = $e->src;

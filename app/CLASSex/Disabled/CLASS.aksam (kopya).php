<?php
	...
	class aksam
	{
		...
		public function rss_fetch($url, $channel = 1)
		{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					$tlink = str_replace('http://www.aksam.com.tr/','', $list[$i]['link']);
					$tlink = explode('/', $tlink);
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					if($_cache->content_cached($list[$i]['link']) == false)
					{
						...
						if(strpos_array($list[$i]['link'], array('http://www.aksam.com.tr/')) == true)
						{
							$parser	= 'aksam_www';
						}
						...
						if($hata <> 1)
						{
							...
							$record = array(
								...
								'cache_object'	=> 'aksam',		//hangi objeyi kullanacağı
		...
		public function aksam_www($data, $type = 'text')
		{
			if($data == '') return '';
			...
			if($type == 'text')
			{
				...
				foreach($html->find('span u') as $e)					$e->outertext = '';

				//en son metni alalım
				foreach($html->find('div#textalan') as $e)				$text = $e->innertext;
			...
			if($type == 'title')
			{
				...
				if($text == '')
				{
					...
					//h1 bulalım
					foreach($html->find('.newsdetailwrap h1') as $e) $text = $e->plaintext;
			...
			if($type == 'desc')
			{
				...
				if($text == '')
				{
					...
					//h2 bulalım
					foreach($html->find('div.newsdetailwrap h2') as $e) $text = $e->plaintext;
			...
			if($type == 'image')
			{
				...
				if($text == '')
				{
					...
					foreach($html->find('div.imagewrapper img') as $e) $text = $e->src;

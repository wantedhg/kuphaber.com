<?php
	...
	class yenimesaj
	{
		...
		public function rss_fetch($url, $channel = 1)
		{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					$tlink = str_replace('http://www.yenimesaj.com.tr/','', $list[$i]['link']);
					$tlink = explode('/', $tlink);
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if($_cache->content_cached($list[$i]['link']) == false)
					{
						...
						if(strpos_array($list[$i]['link'], array('http://www.yenimesaj.com.tr/')) == true)
						{
							$parser	= 'yenimesaj_www';
						}
						...
						$array_url  = array(
							'-makale,',
							'http://www.yenimesaj.com.tr/ehl-i-beyt-gunlugu/',
							'http://www.yenimesaj.com.tr/voleybol/',
							'http://www.yenimesaj.com.tr/motor-sporlari/',
							'http://www.yenimesaj.com.tr/diger/',
						);
						...
						if($hata <> 1)
						{
							...
							$record = array(
								...
								'cache_object'	=> 'yenimesaj',		//hangi objeyi kullanacağı
		...
		public function yenimesaj_www($data, $type = 'text')
		{
			if($data == '') return '';
			...
			if($type == 'text')
			{
				...
				$text = str_replace(array('HABER MERKEZİ'), '', $text);

				//imaj linkleri fullpath olsun
				$text = str_replace('/images/', 'http://www.yenimesaj.com.tr/images/', $text);

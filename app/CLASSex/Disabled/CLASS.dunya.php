<?php
	...
	class dunya
	{
		...
		public function rss_fetch($url, $channel = 1)
		{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					$tlink = str_replace('http://www.dunya.com/','', $list[$i]['link']);
					$tlink = explode('/', $tlink);
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if($_cache->content_cached($list[$i]['link']) == false)
					{
						...
						if(strpos_array($list[$i]['link'], array('http://www.dunya.com/')) == true)
						{
							$parser	= 'dunya_www';
						}
						...
						if($hata <> 1)
						{
							...
							$record = array(
								...
								'cache_object'	=> 'dunya',		//hangi objeyi kullanacağı
		...
		public function dunya_www($data, $type = 'text')
		{
			if($data == '') return '';
			...
			if($type == 'text')
			{
				...
				foreach($html->find('div.source') as $e)				$e->outertext = '';
			...
			if($type == 'desc')
			{
				...
				//h2 bulalım
				foreach($html->find('div.short_content_2') as $e) $text = $e->plaintext;
			...
			if($type == 'image')
			{
				...
				foreach($html->find('img.image_2') as $e) 				$text = $e->src;
				...
				if($text == '')
				{
					...
					//h2 bulalım
					foreach($html->find('.side_detail img.image') as $e) $text = $e->src;

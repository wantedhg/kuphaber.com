<?php
	...
	class aydinlik
	{
		...
		public function rss_fetch($url, $channel = 1)
		{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					$tlink = str_replace('http://www.aydinlikgazete.com/','', $list[$i]['link']);
					$tlink = explode('/', $tlink);
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if($_cache->content_cached($list[$i]['link']) == false)
					{
						...
						if(strpos_array($list[$i]['link'], array('http://www.aydinlikgazete.com/')) == true)
						{
							$parser	= 'aydinlik_www';
						}
						...
						if($hata <> 1)
						{
							...
							$record = array(
								...
								'cache_object'	=> 'aydinlik',			//hangi objeyi kullanacağı
		...
		public function aydinlik_www($data, $type = 'text')
		{
			if($data == '') return '';
			...
			if($type == 'text')
			{
				...
				//en son metni alalım
				foreach($html->find('div#newstext') as $e)					$text = $e->innertext;
				...
				$text = str_replace('/images/', 'http://www.aydinlikgazete.com/images/', $text);
...
// 					$html = str_get_html($data);
//
// 					foreach($html->find('figure.row img') as $e) 	$text = $e->src;
//
// 					bellek boşaltıyoruz
// 					$html->clear();
// 					unset($html);

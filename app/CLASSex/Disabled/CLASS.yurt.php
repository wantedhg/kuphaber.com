<?php
	...
	class yurt
	{
		...
		public function rss_fetch($url, $channel = 1)
		{
			...
			$data_curl = str_replace('http://yurtgazetesi.com.tr','http://www.yurtgazetesi.com.tr',$data_curl);
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					$tlink = str_replace('http://www.yurtgazetesi.com.tr/','', $list[$i]['link']);
					$tlink = explode('/', $tlink);
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if($_cache->content_cached($list[$i]['link']) == false)
					{
						...
						if(strpos_array($list[$i]['link'], array('http://www.yurtgazetesi.com.tr/')) == true)
						{
							$parser	= 'yurt_www';
						}
						...
						if($hata <> 1)
						{
							...
							$record = array(
								...
								'cache_object'	=> 'yurt',			//hangi objeyi kullanacağı
		...
		public function yurt_www($data, $type = 'text')
		{
			if($data == '') return '';
			...
			if($type == 'text')
			{
				...
				$text = str_replace('/images/', 'http://www.yurtgazetesi.com.tr/images/', $text);

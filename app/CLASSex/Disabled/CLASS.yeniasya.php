<?php
	...
	class yeniasya
	{
		...
		public function rss_fetch($url, $channel = 1)
		{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					$tlink = str_replace('http://www.yeniasya.com.tr/','', $list[$i]['link']);
					$tlink = explode('/', $tlink);
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if($_cache->content_cached($list[$i]['link']) == false)
					{
						...
						if(strpos_array($list[$i]['link'], array('http://www.yeniasya.com.tr/')) == true)
						{
							$parser	= 'yeniasya_www';
						}
						...
						$array_url  = array(
							'http://www.yeniasya.com.tr/video/',
							'http://www.yeniasya.com.tr/foto-galeri/',
						);
						...
						if($list[$i]['cat'] == 0 && (date('H:i', strtotime($list[$i]['time']))) == '00:00')
						{
							$hata = 1;
						}
						...
						if($hata <> 1)
						{
							...
							$record = array(
								...
								'cache_object'	=> 'yeniasya',		//hangi objeyi kullanacağı
		...
		public function yeniasya_www($data, $type = 'text')
		{
			if($data == '') return '';
			...
			if($type == 'text')
			{
				...
				//gereksiz tag
				foreach($html->find('cite') as $e)							$e->outertext = '';

				//çok fazla data var ve biz direk sadece onu yakalamak istiyoruz
				$text = $html->find('div.details div.body',0)->innertext;
			...
			if($type == 'title')
			{
				...
				if($text == '')
				{
					...
					//h1 bulalım
					foreach($html->find('div.head h2') as $e) $text = $e->plaintext;
			...
			if($type == 'desc')
			{
				...
				if($text == '')
				{
					...
					foreach($html->find('div.intro') as $e) $text = $e->plaintext;
			...
			if($type == 'image')
			{
				...
				if($text == '')
				{
					...
					foreach($html->find('div.content-image img') as $e) 	$text = $e->src;

					//bellek boşaltıyoruz
					$html->clear();
					unset($html);

					//kısa yol tam yola dönüştürülüyor
					if($text <> '') $text = 'http://www.yeniasya.com.tr'.$text;

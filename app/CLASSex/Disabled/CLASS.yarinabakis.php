<?php
	...
	class yarinabakis
	{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					if($_cache->content_cached($list[$i]['link']) == false)
					{
						...
						if(strpos_array($list[$i]['link'], array('https://www.yarinabakis.com/')) == true)
						{
							$parser	= 'yarinabakis_www';
						}
						...
						if($hata <> 1)
						{
							...
							$record = array(
								...
								'cache_object'	=> 'yarinabakis',	//hangi objeyi kullanacağı
		...
		public function yarinabakis_www($data, $type = 'text')
		{
			if($data == '') return '';

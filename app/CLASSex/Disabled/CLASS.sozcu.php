<?php
	...
	class sozcu
	{
		...
		public function rss_fetch($url, $channel = 1)
		{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					$tlink = str_replace('http://www.sozcu.com.tr/','', $list[$i]['link']);
					$tlink = explode('/', $tlink);
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if($_cache->content_cached($list[$i]['link']) == false)
					{
						...
						if(strpos_array($list[$i]['link'], array('http://www.sozcu.com.tr/')) == true)
						{
							$parser	= 'sozcu_www';
						}
						...
						$array_url  = array(
							...
							'/ilan-2/',
							'/yazarlar/',
							'http://www.sozcu.com.tr/astroloji/',
						);
						...
						if($hata <> 1)
						{
							...
							$record = array(
								...
								'cache_object'	=> 'sozcu',			//hangi objeyi kullanacağı
		...
		public function sozcu_www($data, $type = 'text')
		{
			if($data == '') return '';
...
// 				echo $data;
				...
				//kullanmayacağımız değerleri kaldıralım
				foreach($html->find('div.news-content-share') as $e)		$e->outertext = '';
				foreach($html->find('div.detail-news-spot') as $e)			$e->outertext = '';
				foreach($html->find('div.related-news-box') as $e)			$e->outertext = '';
			...
			if($type == 'desc')
			{
				...
				if($text == '')
				{
					...
					foreach($html->find('div.content-spot') as $e) $text = $e->plaintext;
			...
			if($type == 'image')
			{
				...
				if($text == '')
				{
					...
					foreach($html->find('div.content-img div img') as $e) 	$text = $e->src;

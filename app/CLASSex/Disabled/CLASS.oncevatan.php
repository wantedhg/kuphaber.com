<?php
	...
	class oncevatan
	{
		...
		public function rss_fetch($url, $channel = 1)
		{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					$tlink = str_replace('http://www.oncevatan.com.tr/','', $list[$i]['link']);
					$tlink = explode('/', $tlink);
					...
					$list[$i]['time'] = convert_to_turkish_time( $list[$i]['time'], $hour = '-2 hour');
				}
			}
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if($_cache->content_cached($list[$i]['link']) == false)
					{
						...
						if(strpos_array($list[$i]['link'], array('http://www.oncevatan.com.tr/')) == true)
						{
							$parser	= 'oncevatan_www';
						}

						//belli url mantıklarına izin vermiyoruz
						//galeri içerik veya video dönüyorlar
						$array_url  = array(
							'http://www.oncevatan.com.tr/resmi-ilanlar/',
							'http://www.oncevatan.com.tr/tanitim-yazilari/',
						);
						...
						if($hata <> 1)
						{
							...
							$record = array(
								...
								'cache_object'	=> 'oncevatan',		//hangi objeyi kullanacağı
		...
		public function oncevatan_www($data, $type = 'text')
		{
			if($data == '') return '';
			...
			if($type == 'text')
			{
				...
				foreach($html->find('div[style="align:center"]') as $e) 	$e->outertext = '';

				//en son metni alalım
				foreach($html->find('div#news_container') as $e)			$text = $e->innertext;
			...
			if($type == 'title')
			{
				...
				//h1 bulalım
				foreach($html->find('div#kapsayici h1') as $e) $text = $e->plaintext;
			...
			if($type == 'desc')
			{
				...
				//h2 bulalım
				foreach($html->find('div#kapsayici h2') as $e) $text = $e->plaintext;
			...
			if($type == 'image')
			{
				...
				foreach($html->find('.news_image') as $e) 	$text = $e->src;

<?php
	...
	class turkiye
	{
		...
		public function rss_fetch($url, $channel = 1)
		{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					$tlink = str_replace('http://www.turkiyegazetesi.com.tr/','', $list[$i]['link']);
					$tlink = explode('/', $tlink);
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					if($_cache->content_cached($list[$i]['link']) == false)
					{
						...
						if(strpos_array($list[$i]['link'], array('http://www.turkiyegazetesi.com.tr/')) == true)
						{
							$parser	= 'turkiye_www';
						}
						...
						$array_url  = array(
							'http://www.turkiyegazetesi.com.tr/foto-galeri/',
							'http://www.turkiyegazetesi.com.tr/video-galeri/',
							'http://www.turkiyegazetesi.com.tr/yazarlar/',
						);
						...
						if($hata <> 1)
						{
							...
							$record = array(
								...
								'cache_object'	=> 'turkiye',	//hangi objeyi kullanacağı
		...
		public function turkiye_www($data, $type = 'text')
		{
			if($data == '') return '';
			...
			if($type == 'desc')
			{
				...
				if($text == '')
				{
					...
					foreach($html->find('h2.article_abstract') as $e) $text = $e->plaintext;
			...
			if($type == 'image')
			{
 				preg_match('/<meta itemprop="url" content="([\w\W]*?)" \/>/', $data, $split);
 				$text = strip_tags($split[1]);
				...
				if($text == '')
				{
					...
					//foreach($html->find('img.haber_resim') as $e) $text = $e->src;

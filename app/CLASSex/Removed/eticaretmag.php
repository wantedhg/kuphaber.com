<?php
	...
	class eticaretmag
	{
		public function rss_fetch($url, $type)
		{
			...
			$data_curl = curl_get_data('http://feeds.feedburner.com/EticaretMag');
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					$list[$i]['cache_link']			= trim(strip_tags($data->item[$i]->origLink));
					...
					if($text == '')
					{
						...
						{
							//div kaldıralım
							foreach($html->find('div') as $e)  $e->outertext = '';

							//tüm metni salt metin olarak alalım
							$text = $html->find('p',0)->innertext;
			...
			if($adet > 0)
			{
				...
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if(!in_array($list[$i]['cache_link'], $list_url))
					{
						...
						$array_url  = array(
							'canli-izle',
							'https://www.eticaretmag.com/yazarlar/',
						);
		...
		private function data_fetch($data, $type = 'image')
		{
			...
			if($type == 'image')
			{
				...
				if($html <> '')
				{
					$text = $html->find('div.entry-content img', 0)->src;

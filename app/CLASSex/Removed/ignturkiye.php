<?php
	...
	class ignturkiye
	{
		public function rss_fetch($url, $type)
		{
			...
			$data_curl = curl_get_data('http://tr.ign.com/feed.xml');
			...
			//aylık limitten az içerik yayını yapıldığı için
			//böyle bir limitleme işimizi kolaylaştırıyor
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					//resmin boyutunu yeniden ayarlıyoruz
					if($list[$i]['cache_image'] <> '')
					{
						$list[$i]['cache_image'] = str_replace('.200.','.320.',$list[$i]['cache_image']);
					}
					...
					if($text == '')
					{
						$html = str_get_html(cleanText($data->item[$i]->description));
			...
			if($adet > 0)
			{
				...
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if(!in_array($list[$i]['cache_link'], $list_url))
					{
						...
						$array_url  = array(
							'canli-izle',
							'http://tr.ign.com/video/',
							'http://tr.ign.com/yazarlar/',
						);

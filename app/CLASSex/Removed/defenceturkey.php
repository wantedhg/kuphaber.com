<?php
	...
	class defenceturkey
	{
		public function rss_fetch($url, $type)
		{
			...
			if($html <> '')
			{
				foreach($html->find('div.banner a') as $e) 				$link[] = 'http://www.defenceturkey.com/index2.php'.$e->href;
				foreach($html->find('div.topnews a') as $e) 			$link[] = 'http://www.defenceturkey.com/index2.php'.$e->href;
			...
			if($adet > 0)
			{
				...
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if(!in_array($list[$i]['cache_link'], $list_url))
					{
						...
						$array_url  = array(
							'canli-izle',
							'http://www.defenceturkey.com/yazarlar/',
						);
		...
		private function data_fetch($data, $type = 'image')
		{
			...
			if($type == 'title')
			{
				...
				if($html <> '')
				{
					...
					$text = $html->find('div.article_title',0)->plaintext;
			...
			if($type == 'desc')
			{
				...
				{
					//resmi yakalamaya çalışıyoruz
					$text = $html->find('div.article_content',0)->plaintext;
			...
			if($type == 'image')
			{
				...
				{
					//resmi yakalamaya çalışıyoruz
					$text = $html->find('div.article_picture img', 0)->src;

					$t = $text[0].$text[1].$text[2].$text[3];
					if($t <> '' && $t <> 'http') $text = 'http://www.defenceturkey.com/'.$text;

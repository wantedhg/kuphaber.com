<?php
	...
	class elmadergisi
	{
		public function rss_fetch($url, $type)
		{
			...
			$data_curl = curl_get_data('http://www.elmadergisi.com/feed/');
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					//açıklama alanındaki gereksiz değeri kaldırıyoruz
					$list[$i]['cache_desc'] = str_replace('Continue Reading','',$list[$i]['cache_desc']);
				}
			}
			...
			if($adet > 0)
			{
				...
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if(!in_array($list[$i]['cache_link'], $list_url))
					{
						...
						$array_url  = array(
							'http://www.elmadergisi.com/yazarlar/',
						);
		...
		private function data_fetch($data, $type = 'image')
		{
			...
			if($type == 'image')
			{
				...
				if($html <> '')
				{
					$text = $html->find('img.wp-post-image', 0)->src;

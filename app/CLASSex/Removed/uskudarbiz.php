	...
	class uskudarbiz
	{
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					if($text == '')
					{
						...
						//değişkene atıyoruz
						$list[$i]['cache_image'] = trim(strip_tags($text));
					}
				}
			}
		...
		private function data_fetch($data, $type = 'image')
		{
			...
			if($type == 'image')
			{
				...
				if($html <> '')
				{
					//tüm divleri silerek hile yapıyoruz
					foreach($html->find('div') as $e) $e->outertext = '';

					//kalan metinden resim çıkartıyoruz
					$text = $html->find('img', 0)->src;

					//metne url ekliyoruz
					if($text <> '') $text = 'http://uskudar.biz'.$text;

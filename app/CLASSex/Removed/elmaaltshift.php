<?php
	...
	class elmaaltshift
	{
		public function rss_fetch($url, $type)
		{
			...
			$data_curl = curl_get_data('http://elmaaltshift.com/feed/');
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					//başlık tamamen büyük harflerden oluşmasın
					$list[$i]['cache_title']		= tr_ucwords($list[$i]['cache_title']);
				}
			}
			...
			if($adet > 0)
			{
				...
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if(!in_array($list[$i]['cache_link'], $list_url))
					{
						...
						$array_url  = array(
							'http://www.elmaaltshift.com/yazarlar/',
						);

<?php
	...
	class itnetwork
	{
		public function rss_fetch($url, $type)
		{
			...
			$data_curl = curl_get_data('http://feeds.feedburner.com/itnetworkcom');
			...
			if($adet > 0)
			{
				for($i = 0; $i < $adet; $i++)
				{
					...
					if($text == '')
					{
						...
						//değişkene atıyoruz
						$list[$i]['cache_desc'] = trim(trim(trim($text)));
					}
				}
			}
			...
			if($adet > 0)
			{
				...
				for($i = 0; $i < $adet; $i++)
				{
 					...
 					if(!in_array($list[$i]['cache_link'], $list_url))
					{
						...
						$array_url  = array(
							'http://www.itnetwork.com.tr/yazarlar/',
						);
						...
						if($hata <> 1)
						{
							...
							if($list[$i]['cache_image'] == '')
							{
								...
								//$data = file_get_contents($list[$i]['cache_link']);
								//echo $data;
